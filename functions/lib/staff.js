...
exports.handleCreateStaffAccount = handleCreateStaffAccount;
exports.handleSetStaffRole = handleSetStaffRole;
exports.handleSetStaffActiveStatus = handleSetStaffActiveStatus;
exports.handleDeleteStaffMember = handleDeleteStaffMember;
 ...
 */
async function handleCreateStaffAccount(data, auth) {
    const caller = await (0, authz_1.requireAdminOrOwner)(auth);
    // Role validation
    if (!['trainer', 'admin', 'owner'].includes(data.role)) {
        throw new https_1.HttpsError('invalid-argument', `Invalid staff role "${data.role}". Must be trainer, admin, or owner.`);
    }
    // Admin boundary: Admin may create trainer accounts only
    if (caller.role === 'admin' && data.role !== 'trainer') {
        throw new https_1.HttpsError('permission-denied', 'Admins are authorized to create trainer accounts only.');
    }
    ...
    if (!existingProfileQuery.empty) {
        ...
        if (existingDoc.isActive === true) {
            throw new https_1.HttpsError('already-exists', `An active account already exists with email ${normalizedEmail}.`);
        }
    }
    ...
    try {
        ...
        if (err.code === 'auth/user-not-found') {
            ...
            throw new https_1.HttpsError('internal', `Auth user lookup failed: ${err.message}`);
        }
    }
    ...
    const staffProfileDoc = {
        ...
        fitnessGoal: data.fitnessGoal || (data.role === 'trainer'
 ...
 */
async function handleSetStaffRole(data, auth) {
    const caller = await (0, authz_1.requireOwner)(auth);
    if (!data.targetUid || typeof data.targetUid !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'Target staff UID is required.');
    }
    if (!['trainer', 'admin', 'owner'].includes(data.newRole)) {
        throw new https_1.HttpsError('invalid-argument', `Invalid role "${data.newRole}". Must be trainer, admin, or owner.`);
    }
    const db = admin.firestore();
    const profileRef = db.collection('profiles').doc(data.targetUid);
    const profileSnap = await profileRef.get();
    if (!profileSnap.exists) {
        throw new https_1.HttpsError('not-found', `Staff profile ${data.targetUid} not found.`);
    }
    ...
    if (oldRole === 'owner' && data.targetUid === caller.uid && data.newRole !== 'owner') {
        throw new https_1.HttpsError('invalid-argument', 'You cannot demote your own active owner account.');
    }
    // Update profile
 ...
 */
async function handleSetStaffActiveStatus(data, auth) {
    const caller = await (0, authz_1.requireAdminOrOwner)(auth);
    if (!data.targetUid || typeof data.targetUid !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'Target staff UID is required.');
    }
    if (typeof data.isActive !== 'boolean') {
        throw new https_1.HttpsError('invalid-argument', 'isActive flag must be a boolean.');
    }
    const db = admin.firestore();
    const profileRef = db.collection('profiles').doc(data.targetUid);
    const profileSnap = await profileRef.get();
    ...
    if (caller.role === 'admin') {
        if (targetProfile.role !== 'trainer') {
            throw new https_1.HttpsError('permission-denied', 'Admins are authorized to activate or suspend trainers only.');
        }
    }
    // Owner safe handling: Owner cannot disable themselves
    if (caller.role === 'owner') {
        if (data.targetUid === caller.uid && !data.isActive) {
            throw new https_1.HttpsError('invalid-argument', 'Cannot suspend your own active owner account.');
        }
    }
    // Update profile
    await profileRef.update({
 ...
 */
async function handleDeleteStaffMember(data, auth) {
    const caller = await (0, authz_1.requireOwner)(auth);
    if (!data.targetUid || typeof data.targetUid !== 'string') {
        throw new https_1.HttpsError('invalid-argument', 'Target staff UID is required.');
    }
    if (data.targetUid === caller.uid) {
        throw new https_1.HttpsError('invalid-argument', 'Cannot delete your own active owner account.');
    }
    const db = admin.firestore();
    const profileRef = db.collection('profiles').doc(data.targetUid);
    const profileSnap = await profileRef.get();
